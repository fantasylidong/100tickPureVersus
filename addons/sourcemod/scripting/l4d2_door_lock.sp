...
void Event_OnRoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	delete g_hTimer_PendingLoader;
	delete g_hTimer_WarmingUpTime;
	delete g_hTimer_UnreadyGiveUp;
	delete g_hTimer_CountdownTime;
	delete g_hTimer_ReadyUpChecks;
	delete g_hTimer_ForceVersusST;
}
...
{
	if(!g_bIsGameAllowed || !IsClientInGame(client)) return;
...
int GetReadyTeamCount(int team)
{
	int number = 0;
	for (int i = 1; i <= MaxClients; i++)
		if(IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == team && g_bClientIsReady[i]) number++;
	return number;
}
